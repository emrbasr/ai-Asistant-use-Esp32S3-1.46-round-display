...
// ----------------                                      July 22, 2024                                           ----------------
// ----------------                                                                                              ---------------- 
// ----------------                Used pins SD Card: SD_CLK 14, SD_CMD 17, SD_D0 16                             ----------------
// ----------------                Used pins I2S Microphone INMP441: WS 2, SCK 15, SD 39                         ----------------
...
#define I2S_WS      2         // WS (Word Select/LRCK) pin of INMP441 (L/R pin)
#define I2S_SD      39        // SD (Serial Data) pin of INMP441
#define I2S_SCK     15        // SCK (Serial Clock) pin of INMP441


// --- define your settings ----

#define SAMPLE_RATE             16000  // Ses örnek hızı: 8000, 16000, 24000 veya 44100 Hz
                                       // 16000 Hz Deepgram için optimum değerdir
                                       // Düşük kalite için 8000, yüksek kalite için 24000 kullanabilirsiniz

#define BITS_PER_SAMPLE         16     // 8 veya 16 bit (24/32 bit desteklenmiyor)
                                       // 16 bit daha iyi ses kalitesi sağlar
                                       // 8 bit daha küçük dosya boyutu sunar

#define GAIN_BOOSTER_I2S        32     // INMP441 mikrofon için gain (kazanç) çarpanı
                                       // Değer aralığı: 1-64 (INMP441 için 32 optimum değerdir)
                                       // Yüksek değerler arka plan gürültüsünü artırabilir
...
bool Record_Start( String audio_filename ) 
{
  ...
  { 
    ...
    // Dosya yolunu kontrol et - göreceli yol kullanmak için
    if (audio_filename.startsWith("/")) {
      Serial.println("Dosya yolu '/' ile başlıyor, göreceli yol kullanılacak");
      audio_filename = audio_filename.substring(1);
    }
    
    // Tam dosya yolunu oluştur - kök dizine kaydet
    String fullPath = "/" + audio_filename;
    Serial.println("Kullanılacak tam dosya yolu: " + fullPath);
    ...
    {  
       ...
       Serial.println("Yeni ses dosyası oluşturuluyor: " + fullPath);
    }
    
    // Kalo WAV header
    File audio_file = SD_MMC.open(fullPath, FILE_WRITE);
    if (!audio_file) {
      Serial.println("HATA: WAV dosyası oluşturulamadı! Dosya yolu: " + fullPath);
      // SD kart durumunu kontrol et
      uint8_t cardType = SD_MMC.cardType();
      if(cardType == CARD_NONE) {
        Serial.println("SD kart takılı değil veya tanınmıyor!");
      } else {
        Serial.println("SD kart mevcut, yazma izinleri sorunu olabilir.");
      }
      return false;
    }
    ...
    Serial.println("> WAV Header oluşturuldu, ses kaydı başladı ... ");
    return true;
  }
  ...
  { 
    ...
    File audio_file = SD_MMC.open(fullPath, FILE_APPEND);
    if (!audio_file) {
      Serial.println("ERROR in Record_Start() - Failed to open audio file! Path: " + fullPath);
      return false;
    }
  ...
  return false; // Bu satır hiçbir zaman çalışmayacak, ama compiler hata vermesin diye eklendi
}
...
bool Record_Available( String audio_filename, float* audiolength_sec ) 
{
  ...
  { 
    ...
    File audio_file = SD_MMC.open(fullPath, "r+");
    if (!audio_file) {
      Serial.println("HATA: WAV dosyası güncellenemedi! Dosya yolu: " + fullPath);
      ...
      if(cardType == CARD_NONE) {
        ...
        if (SD_MMC.exists(fullPath)) {
          Serial.println("Dosya mevcut, ancak açılamıyor (yazma izni sorunu?)");
        } else {
      ...
      *audiolength_sec = 0.0;
      return true; // Kaydı durdurmak için true döndür
    }
    ...
    *audiolength_sec = (float) (filesize-44) / (SAMPLE_RATE * BITS_PER_SAMPLE/8);   // return Audio length (via reference) 
    ...
    Serial.print("> New AUDIO file: '" + fullPath + "', filesize [bytes]: " + (String) filesize);
    Serial.println(", length [sec]: " + (String) *audiolength_sec);
